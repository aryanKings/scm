...
<nav style="margin-top:95px;"  class="navbar navbar-default">
  <div class="container-fluid">
    ...
    <li class="headerlinks"  ><a style=""  href="WContractor.php" >Works for contract</a></li>
    ...
    <li class="headerlinks" ><a style=""  href="WMaterialSuppliers.php" >Material Supplier</a></li>
    <li class="headerlinks" ><a style=""  href="WManpowerSuppliers.php">Manpower Supplier</a></li>
      <li class="headerlinks" ><a style=""  href="WFAQ.php">FAQ</a></li>
...
<div class="container" style="margin-bottom:100px">
 <center> <strong><h3 style="font-weight: 700;margin-bottom:30px">Works for Contract List</h3></strong></center>
 <center><div class="span12" style="background-color: #eee;">
            <form method="post" action="WorksSubContractView.php" class="form-inline" style="padding-top: 35px; padding-bottom: 35px;" >
            ...
            <input type="text" name="search_box1" oninvalid="InvalidMsg(this);" oninput="InvalidMsg(this);" class="form-control input-sm" maxlength="10" placeholder="Enter work" />
...
/*
 * data for work for sub contractor post
 */
    ...
    if(isset($_POST['search'])){
        $location =  htmlspecialchars($_POST['search_box']);
        $work = htmlspecialchars($_POST['search_box1']);
        
        $row =  $database->getRowsDatabase("SELECT * FROM  postwork WHERE location LIKE '%$location' OR work LIKE '%$work%' ORDER BY pwid DESC");
    }else{
        $row =  $database->getRowsDatabase("SELECT * FROM postwork ORDER BY pwid DESC");    
    }
   ...
   if($row != null){ 
                for($i = 0; $i < count($row); $i++){?>
                    <li class="col-sm-6 col-xs-12">
                        ...
                        <h3>Work: <?php echo $row[$i][9]; ?> </h3>
                        <header>
                            <?php 
                                    $userids =  $row[$i][1];
                                    $company =  $database->getRowsDatabase("SELECT company FROM profile WHERE userid='$userids'");
                             ?>
                        <p class="clearfix"><span class="company-name">Company: <?php if($company !=  null){ echo  $company[0][0];}?></span></p>
                        </header>
                        <p class="job-info">
                        <span class="location-info"><i class="fa fa-map-marker"></i>Location: <?php echo  $row[$i][3];?></span>
                        <span class="detail"><strong>Description:</strong> Work Description: <?php echo  $row[$i][2];?></span>
                        </p>
                        <div class="apply">
                        <center><button style="margin-bottom: 12px;width:115px;" type="submit" class="btn btn-default">Apply</button></center>
                        </div>
                        <footer>
                        <p class="clearfix">
                        <span class="experience"><i class="fa fa-user"></i>Approximate Value of Work: <?php echo  $row[$i][7];?></span>
                         <span class="metadata">Last Date: Mar 31<span class="glyphicon glyphicon-calendar"><?php echo  $row[$i][8];?></span></span>
                        </p>
                        </footer>
                        </div>
                 </li>
...
</div><footer style="margin-bottom:0px" class="navbar navbar-fixed-bottom footerbottom">
